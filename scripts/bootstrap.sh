...
echo "${BLUE}Installing mcp-auth-server-base package...${NC}"
echo ""
...
if [ -f "./agent/scripts/acp.package-install.sh" ]; then
    ...
    if [ $? -eq 0 ]; then
        echo ""
        echo "${GREEN}✓${NC} mcp-auth-server-base package installed successfully"
        echo ""
    else
        echo ""
...
echo "  • mcp-auth-server-base package"
echo ""
echo "Next steps:"
echo "  1. Run: @acp.init"
